...
Deaf_GD_Data <- subset(Deaf_Data, FFD_ms >= Min_FFD & GD_ms <= Max_GD)
Deaf_Filtered <- 1 - length(Deaf_GD_Data$GD_ms)/length(which(Deaf_Data$FFD_ms > 0))
Deaf_Filtered
...
Deaf_GD_GLMM_1 <- glmer(GD_ms ~ Condition * Predictability_Value_Z * Reading_Score_Z + (1 | Participant) + (1 | Frame_ID), data = Deaf_GD_Data,
                        Gamma(link = "identity"),
                        glmerControl(optimizer = "bobyqa"))

summary(Deaf_GD_GLMM_1)