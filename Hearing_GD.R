...
Hearing_GD_Data <- subset(Hearing_Data, FFD_ms >= Min_FFD & GD_ms <= Max_GD)
Hearing_Filtered <- 1 - length(Hearing_GD_Data$GD_ms)/length(which(Hearing_Data$FFD_ms > 0))
Hearing_Filtered
...
Hearing_GD_GLMM_1 <- glmer(GD_ms ~ Condition * Predictability_Value_Z + (1 | Participant) + (1 | Frame_ID), data = Hearing_GD_Data,
                        Gamma(link = "identity"), nAGQ=1,
                        glmerControl(optimizer = "bobyqa"))

summary(Hearing_GD_GLMM_1)